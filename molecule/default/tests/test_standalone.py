...
testinfra_hosts = testinfra.utils.ansible_runner.AnsibleRunner(
    os.environ['MOLECULE_INVENTORY_FILE']).get_hosts('cassandra_standalone')
...
def test_config_file(host):
    ...
    assert f.exists
    assert f.is_file
    assert f.contains('cluster_name: Standalone')
...
def test_cluster_name(host):
    ...
    assert cmd.rc == 0
    matches = re.findall('Name: Standalone', cmd.stdout)
